...
void solve(int tc)
{
    ...
    sort(v.begin(),v.end());
    auto itr1 = lower_bound(v.begin(),v.end(),mp(a,0));
    auto itr2 = upper_bound(v.begin(),v.end(),mp(b,(int) v.size()-1));
    vector<int> p;
    for(auto itr = itr1; itr != itr2; itr++){
        p.push_back((*itr).second+1);
    }
    if(flag){
        for(int i = 0;i<p.size();i++){
            p[i] = ((int) s.size()+1)-p[i];
        }
        reverse(all(p));
    }
    cout << b-a << " " << (itr2)-(itr1) << "\n";
    ...
    //     flag = true;
	// }
	// auto itr1 = st.lower_bound({a,-1});
	// auto itr2 = st.lower_bound({b,1e6});
	// int j = 0;
	// int ans = 0;
	// vector<int> p;
	// j--;
