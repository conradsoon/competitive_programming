...
struct aO
{
	...
	int r;
	bool hasPendingOp;
	aO() : a(0), d(0), l(-1), r(-1), hasPendingOp(false)
	{
	}
	aO(ll a, ll d, int l, int r) : a(a), d(d), l(l), r(r), hasPendingOp(true)
	{
	}
	void setNewOp(aO &newOp)
	{
		a = newOp.a;
		d = newOp.d;
		hasPendingOp = newOp.hasPendingOp;
	}
	void compose(aO &parentOp)
	{
	}
	iAM applyAggrOp(iAM nodeVal)
	{
		nodeVal.val += a * (r - l + 1);
...
struct node
{
	...
	void compose(node &pOp)
	{
		lazyOp.compose(pOp);
	}
	void applyAggrOp()
	{
		lazyOp.applyAggrOp(val);
