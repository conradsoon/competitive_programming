...
int rn = 48;
vvi comprot(rn, vi(48, -1));
void compcorrn(vb &procdCorrn, vector<corr> &corrns, int idx)
{
	if (procdCorrn[idx])
	{
		return;
	}
	compcorrn(procdCorrn, corrns, corrns[idx].par);
	procdCorrn[idx] = true;
	corrns[idx].r = comprot[corrns[corrns[idx].par].r][corrns[idx].r];
	corrns[idx].del = add(corrns[corrns[idx].par].del, rot(corrns[idx].del, corrns[corrns[idx].par].r));
}
int main()
{
	...
	forn(i, 1, nb)
	{
		for (auto pt : s[i])
		{
			pts.insert(add(corrns[i].del, rot(pt, corrns[i].r)));
		}
	}
	cout << sz(corrns);
	return 1;
